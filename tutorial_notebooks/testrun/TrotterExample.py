    ...
    def executor(circuits):
        if do_cross_talk_noise:
            ...
            if do_multiprocessing:
                ...
                new_circuits = []
            # For some reason pickleing every circuit indiviually and sending it via the process is WAY slower that pickleing all at once and sending over the pickle file
            pickle_file_name = namebase+"temp_circuits.pickle"
            ...
            if do_multiprocessing:
                ...
                for i in range(len_circuits):
                    ...
                    print("Opening process %s/%s" % (i+1, len_circuits), "; Number of Threads: %03s" % len(multiprocessing.active_children()), end='\r')
                    process = multiprocessing.Process(target=apply_cross_talk, args=(i, pickle_file_name, new_circuits, backend, folder))
                    process.start()
                    while len(multiprocessing.active_children()) > multiprocessing.cpu_count():
                        pass
            else:
                for circ in circuits:
                    print("Processing Circuit %s/%s" % (circuits.index(circ)+1, len(circuits)), end='\r')
                    apply_cross_talk(circ, new_circuits, backend, transpile, folder)
...
def apply_cross_talk(i, pickle_file_name, new_circuits, backend, folder):
    import pickle
